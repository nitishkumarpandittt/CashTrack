...
// "Error connecting to database". Every write in this app runs from the
// browser, so this is the only place the real reason ever becomes visible.
...
const POSTGRES_HINTS = {
  ...
  "42P01":
    "That table does not exist in this database. Run `npm run db:push` against the same NEXT_PUBLIC_DATABASE_URL the app was built with.",
  "42703": "The table is missing a column the app expects. Run `npm run db:push` against this database.",
  "42501": "The database role in NEXT_PUBLIC_DATABASE_URL is not allowed to write to this table.",
  "25006": "This connection is read-only (a read replica or read-only role), so nothing can be saved.",
  "28P01": "Database authentication failed: the password in NEXT_PUBLIC_DATABASE_URL is wrong or was rotated.",
  "28000": "Database authentication failed: check the role and password in NEXT_PUBLIC_DATABASE_URL.",
  "3D000": "The database named in NEXT_PUBLIC_DATABASE_URL does not exist.",
...
export function describeDbError(error) {
  ...
  if (error.sourceError || TRANSPORT_PATTERN.test(message)) {
    ...
    return `Could not reach the database (${reason}). Check that NEXT_PUBLIC_DATABASE_URL points at a live Neon endpoint and that the deployment was rebuilt after it changed.`;
  }
