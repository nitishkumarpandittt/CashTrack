...
export default function LiquidEther({
  ...
  // Upper bound on devicePixelRatio. The default matches the original
  // component; a diffuse background can safely run at 1, which is a 4x cut in
  // fragment work on a HiDPI display.
  maxPixelRatio = 2
}) {
...
`;
    ...
    // A resize reallocates all seven WebGLRenderTargets, so it must only run
    // when the box has genuinely changed. During page load the host element
    // reflows repeatedly (font swap, entrance animations, images settling) and
    // firing on every one of those caused a burst of GPU reallocations —
    // visible as jitter while the page settles.
    let lastW = 0;
    let lastH = 0;
